...
const subjects: Subject[] = [
  ...
  { name: "Computer Science", icon: Code, color: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300" },
]
      ...
      contents: [
        {
          role: "user",
          parts: [
            { text: "You are EduBot, a friendly AI tutor that helps students learn subjects. Keep explanations simple and clear." },
            { text: trimmed },
          ],
        },
      ],
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {quickQuestions.map((question, index) => (
                  <Badge
                    key={index}
                    variant="secondary"
                    className="cursor-pointer hover:bg-primary hover:text-primary-foreground px-4 py-2 rounded-full"
        ...
        <Card className="flex flex-col rounded-xl shadow-lg">
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            {/* Messages */}
            <div className="flex-1 pr-4 overflow-auto max-h-[70vh]" ref={scrollAreaRef}>
              <div className="space-y-4 p-2">
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="bg-muted p-4 rounded-xl shadow-sm flex items-center gap-2">
                      <div className="w-2 h-2 bg-blue-600 rounded-full animate-bounce"></div>
                      <div className="w-2 h-2 bg-blue-600 rounded-full animate-bounce" style={{ animationDelay: "0.1s" }}></div>
                      <div className="w-2 h-2 bg-blue-600 rounded-full animate-bounce" style={{ animationDelay: "0.2s" }}></div>
                      <span className="text-sm text-muted-foreground ml-2">EduBot is thinking...</span>
                    </div>
                  </div>
                )}
              </div>
